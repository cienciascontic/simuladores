...
to setear
  ca
  ask patches [set pcolor lime + 3]
    reset-ticks
  create-conejos cant-inicial-conejos
  [
    set shape "conejo"
    set size 2
    set color white
    setxy random-xcor random-ycor
    set energía (random 20) / 10
  ]

  create-plantas cant-inicial-plantas
  [
    set shape "planta"
    set size 1.5
    set color green - 3
...
to ejecutar
  if count conejos = 0 OR ticks >= 1000 OR count turtles > 1000 [stop]
  ask conejos [conejos-moverse set energía energía - 0.01]
  conejos-alimentarse
  conejos-reproducirse
  conejos-morirse
  tick
end
...
to conejos-reproducirse
  ask conejos [
    if energía > 2.4
    [
      set energía energía - 2.4
      hatch 1
      [
        set shape "conejo" set size 2 set color brown setxy random-xcor random-ycor
        set energía 1 + (random 10 ) / 10
      ]
    ]              ]
end
...
"" ""
PENS
"plantas" 1.0 0 -12087248 true "" "plot count plantas"
"conejos" 1.0 0 -2674135 true "" "plot count conejos"
